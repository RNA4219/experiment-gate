...
from experiment_gate.request_loader import build_request_from_payload
from experiment_gate.result_formatter import build_agent_result
from experiment_gate.runner import run, run_gate
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Run command (legacy insight mode)
    run_parser = subparsers.add_parser("run", help="Run insight analysis (legacy)")
    ...
    run_parser.add_argument("--set", dest="set_values", action="append", default=[], help="Override config value")
    ...
    run_parser.add_argument("--include-source-units", action="store_true", help="Include source units in output")
...
def run_insight_command(args: argparse.Namespace) -> int:
    """Handle the 'run' (legacy insight) subcommand."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "gate":
        ...
        return run_insight_command(args)
    else:
        parser.print_help()
        return 1
