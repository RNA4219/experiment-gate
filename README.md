...
# サブコマンド省略でも実行可能
python -m experiment_gate -i examples/sample_gate_request.json
...
# Gate 設定ファイルを指定
python -m experiment_gate gate -i request.json --config config/defaults.yaml

# Gate 閾値をその場で上書き
python -m experiment_gate gate -i request.json --set gate.scoring.thresholds.go_min=80
```

### Python API

```python
from experiment_gate import GateRequest, PocSpec, EvidenceBundle, run_gate
...
print(response.decision.verdict.value)
```
